...
| 文件 | 场景 | 收录日期 |
|------|------|----------|
| `communication.md` | 沟通：中文回答、结论先行、区分事实与推断、直接推荐最佳方案 | 2026-08-18（增补 08-19） |
| `evidence-research.md` | 证据与检索：先搜索验证、一手来源、网页不可信、结论锚定代码位置 | 2026-08-18（增补 08-19） |
| `local-work.md` | 本地操作：先查真实状态、最小修改、先诊断后修复 | 2026-08-18 |
| `bug-fixing.md` | 根因修复：验证根因、最小修复、边界场景豁免 | 2026-08-19 |
| `performance-tuning.md` | 性能调优：测量先行、数据锚定、复杂度预算 | 2026-08-19 |
| `code-convergence.md` | 收敛与清理：单一实现、清理独立提交 | 2026-08-19 |
...
> 2026-08-19 提炼自 [blog.52013120.xyz SKILL「核心原则」](https://blog.52013120.xyz/tools/skills/SKILL.html)：
> 采纳其根因纪律、代码锚定、收敛原则；弃用其绝对化的「避免兜底逻辑」表述（边界关键场景加豁免）；
> 其「最少代码」目标函数不适用性能场景，`performance-tuning.md` 改写为复杂度预算。新增三份与增补两份均**待会话验证**。
